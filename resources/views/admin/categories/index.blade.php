                            ...
                            <tr class="group hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="inline-flex items-center justify-end gap-1 rounded-lg bg-zinc-100 p-1 dark:bg-zinc-800">
                                        ...
                                        <form action="{{ route('dashboard.categories.toggleActive', $category->id) }}" method="POST" class="inline-flex">
                                            @csrf @method('PATCH')
                                            <button type="submit" title="{{ $category->is_active ? 'Desactivar' : 'Activar' }}" class="rounded-md p-1.5 text-zinc-500 hover:bg-white {{ $category->is_active ? 'hover:text-amber-500' : 'hover:text-emerald-600' }} hover:shadow-sm dark:text-zinc-400 dark:hover:bg-zinc-700 transition-all">
                                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    @if($category->is_active)
                            ...
                            @foreach($categories->where('parent_id', $category->id) as $sub)
                                <tr class="group hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                                    ...
                                    <td class="px-6 py-3 text-right">
                                        <div class="inline-flex items-center justify-end gap-1 rounded-lg bg-zinc-100 p-1 dark:bg-zinc-800">
                                            <form action="{{ route('dashboard.categories.toggleActive', $sub->id) }}" method="POST" class="inline-flex">
                                                @csrf @method('PATCH')
                                                <button type="submit" title="{{ $sub->is_active ? 'Desactivar' : 'Activar' }}" class="rounded-md p-1.5 text-zinc-500 hover:bg-white {{ $sub->is_active ? 'hover:text-amber-500' : 'hover:text-emerald-600' }} hover:shadow-sm dark:text-zinc-400 dark:hover:bg-zinc-700 transition-all">
                                                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                        @if($sub->is_active)
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
